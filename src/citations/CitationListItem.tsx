        ...
        <section className={styles.listItem}>
            ...
            <div className={styles.content}>
                <summary className={styles.title}>
                    <Link to={url}>{title ? title : rawText}</Link>
                </summary>
                <div className={styles.detailLine}>
                    {authors ? authors.map(({ first, last, middle, raw }) => first && last ? `${last}, ${first}` : raw).join(' | ') : ''}
                </div>
                <div className={styles.detailLine}>
                    {location}{location && datePublished ? ', ' : ''}{datePublished}
                </div>
                <div className={styles.detailLine}>TA {volume}, {number}</div>
                <div className={styles.keywordsLine}>{keywords.map(
                    ({ nameEN, raw, code }) => (
